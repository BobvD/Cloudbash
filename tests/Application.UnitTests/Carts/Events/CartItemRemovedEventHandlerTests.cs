...
namespace Cloudbash.Application.UnitTests.Carts.Events
{
    ...
    public class CartItemRemovedEventHandlerTests : EventHandlerTestBase
    {
        private Guid _ticketTypeId = new Guid("4f356bc0-ebe0-409a-aea7-d7fdd7b76154");

        public CartItemRemovedEventHandlerTests(TestFixture fixture) : base(fixture)
        {
        }
        ...
        public async Task When_Concert_Created_Event_Handled_ConcertVm_Saved()
        {
            ...
            CartItemRemovedEvent @event = (CartItemRemovedEvent)cart.GetUncommittedEvents().Last();

            var handler = new CartItemRemovedEventHandler(_cartRepo);

            var notification = new DomainEventNotification<CartItemRemovedEvent>(@event);
